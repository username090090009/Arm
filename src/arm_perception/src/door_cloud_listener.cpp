...
private:
    ...
    void cloudCallback(const sensor_msgs::PointCloud2ConstPtr& msg)
    {
        ...
        // 10. Make approach pose in camera frame
        geometry_msgs::PoseStamped approach_pose_camera =
            makeCameraApproachPose(center, coefficients, msg->header.frame_id);

        // 11. Transform approach pose to base_link
        geometry_msgs::PoseStamped approach_pose_base;
        if (transformPose("base_link", approach_pose_camera, approach_pose_base))
        {
            // 12. Build orientation in base_link using transformed normal
            ...
            {
                approach_pose_base.pose.orientation.x = 0.0;
                approach_pose_base.pose.orientation.y = 0.0;
                approach_pose_base.pose.orientation.z = 0.0;
                approach_pose_base.pose.orientation.w = 1.0;
            }
