...
public:
        ...
        executed_once_(false)
    {
        ...
        pnh.param<double>("position_tolerance", position_tolerance_, 0.01);
        pnh.param<double>("orientation_tolerance", orientation_tolerance_, 0.10);
        pnh.param<double>("planning_time", planning_time_, 8.0);
        pnh.param<int>("max_attempts", max_attempts_, 10);

        // 喷涂参数：主方向从上往下
        pnh.param<double>("spray_width", spray_width_, 0.06);     // 左右短摆
        pnh.param<double>("spray_height", spray_height_, 0.12);   // 上下主方向
        pnh.param<double>("line_spacing", line_spacing_, 0.03);   // 每行向下步进
...
private:
    void targetCallback(const geometry_msgs::PoseStampedConstPtr& msg)
    {
        ...
        {
            ROS_INFO_THROTTLE(2.0, "Spray task already executed once. Ignoring repeated updates.");
        }
    }
    ...
    std::vector<geometry_msgs::Pose> generateVerticalSnakeWaypoints(const geometry_msgs::PoseStamped& center_pose)
    {
        ...
        geometry_msgs::Quaternion fixed_orientation = center_pose.pose.orientation;
